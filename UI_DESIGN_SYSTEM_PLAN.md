...
**Not yet migrated, out of the original Stage 5 A-D scope:** `settings/ai`, `settings/organization`, and `settings/billing` pages, plus `all-apps/ledger`'s own page content, still use `SettingsCard` and were never enumerated in any of the four groups above (Group D was "Home dashboard & auth polish" specifically, not the full Settings cluster). The plan's closing cleanup line ("delete `SettingsCard` once all consumers move to `Card`") is still open -- `SettingsCard` currently has 6 consumers, not the 4 the plan text estimated when it was written. Left as a deliberate follow-up rather than silently expanding Group D's scope.

**Stage 5 Group C notes:** the 5 report pages' "Print" buttons now use `ui/Button`, and `drill-down-panel.tsx`'s back/close icon buttons now use `ui/IconButton` (that file turned out to be dead code, not imported anywhere -- its edit is typechecked but unverifiable live). The report tables themselves (`trial-balance-page.tsx`, `reports-page.tsx`, `pl-detail-page.tsx`, `by-payee-page.tsx`, and the shared `ReportSection`/`ReportAccountRows` row renderers) were deliberately kept as raw `<table>`/`<tr>`/`<td>` rather than forced onto `Table.*` primitives: they're print-optimized financial statements already wrapped in their own `.report-print-card` border/padding, with exact per-row background treatments for section headers and grand totals that `Table.Root`'s generic bordered-scrollable-div and `Table.Row`'s hover state would conflict with -- the same reasoning that kept `register-table.tsx` raw in Group A. No native selects/checkboxes/radios/textareas remained anywhere in the reports cluster. Verified live: Trial Balance, Profit and Loss, By Payee, and P&L Detail all render correctly with the new Print button, plus a clean `tsc --noEmit` and `next build`.
