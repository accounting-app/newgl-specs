...
**Status:** Stage 3 done (quickslike commit `65a868a`, branch `21-issue_ui_fixes_and_updates_dc`). Stages 4-5 not started.

**Stage 3 notes:** built richer than the original flat-pill sketch in Part 3, per user-provided QBO screenshots of the actual "All apps" menu -- implemented as a real two-level hover flyout (category list + nested sub-items panel), matching QBO's interaction exactly, still with zero locked/"coming soon" placeholders. `src/constants/apps.ts` correctly places every existing screen (reviewed the full route list to confirm none were missed): Accounting = Register + Chart of Accounts + Bank Rules + Ledger, Reports = its own established sub-nav (unchanged), AI = AI settings, Account = Billing + Organization. Settings sidebar and home dashboard pills both now derive from this instead of owning independent copies. Register intentionally kept as its own top-level rail shortcut (not absorbed into a persistent Accounting sidebar) -- it's flagged as the highest-risk migration target in Stage 5 Group A and forcing it into the settings-sidebar layout now would be scope creep ahead of that pass.
