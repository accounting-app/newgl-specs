...
**Decision: intentionally deferred.** The specific provider will be chosen when Phase 3 actually starts; the port stays swappable via `AI_PROVIDER` so this doesn't block Phases 1-2.
...
### New infra
- `newgl-api/src/infra/imports/import-store.ts`: in-memory batch store + TTL cleanup (**24 hours**, confirmed), raw file staging under `data/imports/`.
...
- `newgl-api/src/infra/imports/ofx-parser.ts`: OFX is structured SGML/XML — deterministic parse, **no AI needed**. **Decision: hand-rolled, minimal parser, no new dependency** (confirmed — consistent with the project's "no unnecessary deps" style).
- `newgl-api/src/infra/imports/ai-statement-parser.ts`: port (`interface AiStatementParser { extract(file: Buffer, mimeType: string): Promise<ParsedRow[]> }`) + one concrete adapter (provider TBD — see "AI provider" decision above) selected via `AI_PROVIDER` env var in `newgl-api/src/configuration/index.ts`.

### New pure logic in `core/` (testable, no I/O — matches existing style)
- `newgl-api/src/core/account-suggestion.ts`: score existing accounts (from `Account["name"]`/`subtype`) plus historical `Transaction.accountLabel`/`payee` pairs (via `TransactionService.listTransactions()`) against each row's description; keyword/substring/Jaccard scoring. **Decision: ships in Phase 1** (confirmed — basic keyword-based suggestion is not AI-dependent, so there's no reason to gate it behind Phase 3/4), upgradeable to historical-learning/LLM classification later (Phase 4).
...
- **Navigation: new top-level nav item "Import"** (confirmed) — add an entry in `quickslike/src/components/layout/side-nav.tsx` alongside the existing top-level items (e.g. Register, Reports), linking to `/import`.
...
- **Phase 1 (MVP):** CSV parser + basic keyword-based account-suggestion (confirmed for Phase 1) + manual account mapping/override, no AI, no OFX. Validates the whole staging/commit pipeline end-to-end with the lowest-risk parser.
- **Phase 2:** OFX parser (hand-rolled, deterministic, no new dependency, no AI dependency — confirmed).
- **Phase 3:** AI vision parsing for PDF and image files via the pluggable `AiStatementParser` port; confidence flagging and partial-failure UX. Provider selection happens at the start of this phase.
...
The following questions were asked to resolve the open decisions from the first draft of this plan, with answers recorded below:

1. **Which AI provider should we use in Phase 3 for parsing PDFs/images (vision + structured output)?**
   Answer: **Defer** — decide when Phase 3 actually starts. The port (`AiStatementParser`) stays swappable via `AI_PROVIDER` so this doesn't block earlier phases.

2. **What should we do right now with this plan?**
   Answer: **Only wait for further instructions** — the priority right now is having a well-defined `.md` file; no implementation should start yet.

3. **Should automatic target-account suggestion be included already in Phase 1, or deferred?**
   Answer: **Include a basic keyword-based suggestion starting in Phase 1** (it doesn't depend on AI, so there's no reason to gate it behind a later phase).

4. **For the OFX parser (Phase 2), hand-rolled or a library?**
   Answer: **Hand-rolled, minimal parser, no new dependency.**

5. **Where should the import wizard live in the frontend navigation?**
   Answer: **New top-level nav item "Import"** (added to `quickslike/src/components/layout/side-nav.tsx`).

6. **How long should unconfirmed import batches/files be retained before automatic cleanup?**
   Answer: **24 hours.**

## Current status

This remains a **design-only plan** — no implementation has started. Per the answer to question 2 above, the next step is to wait for an explicit go-ahead before starting Phase 1.

## Remaining open decision

- AI provider for Phase 3 (OpenAI vs Anthropic vs Google) — intentionally deferred until Phase 3 starts (see Q&A above).
