...
**Status: done.** No backend changes, as anticipated. Extracted `splitPastedRows` (tab/comma row-splitting) out of `journal-entry-modal.tsx` into the shared `parse-csv.ts` so it's not duplicated between Journal Entry's paste handler and this new one. New "Bulk paste import" card on Settings → Ledger (a ledger-wide page, not account-scoped, unlike the CSV wizard/Journal Entry toolbar buttons which live inside a specific account's register): paste Date/Account/Offset Account/Amount/Memo rows (optional header row, else that fixed column order), each becomes its own review-table row with independent account pickers, then submits as its own `createTransaction` + `postTransaction` call per row (explicit sign convention: positive Amount debits Account and credits Offset Account, negative reverses it -- this feature's own rule, not inherited from the backend, which only enforces balanced postings). Account name matching reuses Journal Entry's exact case-insensitive label lookup, falling back to a manual `SelectField` per row for anything unmatched, exactly like the "Unmatched account" fallback already used there. Verified live: pasted 3 rows (two valid, one with a misspelled account and one incomplete), confirmed unmatched accounts were flagged and excluded from the ready count, manually resolved one via the row's account picker, submitted 2 transactions successfully, and confirmed both posted correctly in the register with the right debit/credit direction -- the failed/incomplete row correctly stayed in the table for retry instead of being silently dropped.
